...
_telemetry-packet-member-sequence_ is a
<<Specifiers_Telemetry-Packet-Specifiers,telemetry packet member
sequence>>: an <<Element-Sequences,element sequence>> in which each
element is either a _non-annotatable-include-specifier_ or a
...
.. The `omit` clause is present, and _T_ appears in
_telemetry-packet-member-sequence_ after FPP recursively resolves any
include specifiers there.
